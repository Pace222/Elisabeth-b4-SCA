...
class B4GaussianEstimator:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        for lab in self.unique_y:
            ...
            self.templates[np.where(self.unique_y == lab)[0][0]] = multivariate_normal(mean, cov)
...
def b4_train(sig_strength: SignalStrength, n_feat: int, traces_train: np.ndarray, labels_train: np.ndarray):
    sig_strength.fit(traces_train, labels_train, n_feat)
    features_train = sig_strength.transform(traces_train)
    gaussian_est = B4GaussianEstimator().fit(features_train, labels_train)
