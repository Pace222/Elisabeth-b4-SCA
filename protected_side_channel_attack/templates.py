...
def b4_gridsearch_cv(sig_strength: SignalStrength, n_folds: int, train_size: int, num_features: List[int], traces_trainval: np.ndarray, labels_trainval: np.ndarray):
    ...
    for cv, (train_index, val_index) in enumerate(rs.split(traces_trainval)):
        ...
        for n, n_feat in enumerate(num_features):
            ...
            results[n, cv] = np.count_nonzero(predicted == labels_val) / labels_val.shape[0]
