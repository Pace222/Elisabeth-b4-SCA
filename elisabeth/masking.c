...
packed masked_sbox_first_order(packed inp_shares, const uint32_t* s_box) {
    ...
    uint4_t off_left = 8 * (r_prime % 4), off_right = 32 - off_left;

    packed output_shares = get_rand();
    uint4_t neg_out_summed_4 = uint4_neg(SHARE_0(output_shares));
    uint32_t neg_out_summed_32 = (neg_out_summed_4 << 24) | (neg_out_summed_4 << 16) | (neg_out_summed_4 << 8) | neg_out_summed_4;
    ...
    for (uint4_t a = 0, curr = r_prime / 4; a < 0x10; a += 4, curr = (curr + 1) % 4) {
        ...
        t[curr] |= (s >> off_left);
        t[(curr + 1) % 4] |= (s << off_right);
    }

    uint4_t idx = uint4_add(r, inp_1 % 4);
    return (output_shares & MASK_0) | uint4_new(t[idx / 4] >> (8 * (3 - (idx % 4))));
}
